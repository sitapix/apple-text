...
def ensure_node_deps(force: bool) -> None:
    ...
    ensure_package_node_deps(ROOT, npm=npm, force=force, label="root")

    if MCP_ROOT.exists():
        ensure_package_node_deps(MCP_ROOT, npm=npm, force=force, label="mcp-server")


def ensure_package_node_deps(package_root: Path, *, npm: str, force: bool, label: str) -> None:
    node_modules = package_root / "node_modules"
    if node_modules.exists() and not force:
        print(f"{label} node_modules already present")
        return

    install_command = [npm, "ci"] if (package_root / "package-lock.json").exists() else [npm, "install"]
    print(f"Installing {label} Node dependencies with {' '.join(install_command)}...")
    run(install_command, cwd=package_root)
