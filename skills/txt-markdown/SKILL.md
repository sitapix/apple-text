...
- Regex/parsing mechanics are the main question -> `/skill txt-parsing`
...
- Use `/skill txt-attributed-string` for attribute-model and conversion choices.
- Use `/skill txt-swiftui-bridging` when the real gap is SwiftUI rendering limits.
- Use `/skill txt-parsing` when the problem is parser choice more than Markdown rendering semantics.
