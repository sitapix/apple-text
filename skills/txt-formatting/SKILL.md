...
- Need type choice or custom attribute scopes -> `/skill txt-attributed-string`
- Need exact formatting keys and compatibility -> stay here
- Need semantic text colors rather than general formatting -> `/skill txt-colors`
...
- Use `/skill txt-attributed-string` for AttributedString vs NSAttributedString decisions.
- Use `/skill txt-colors` when the formatting question is mostly about semantic color behavior.
- Use `/skill txt-attachments` for inline non-text content instead of pure formatting attributes.
