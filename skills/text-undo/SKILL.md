...
- Need storage editing lifecycle -> `/skill text-storage`
- Need layout invalidation after undo -> `/skill text-layout-invalidation`
- Need general debugging -> `/skill text-textkit-diag`
...
- Use `/skill text-storage` for the editing lifecycle behind undo recording.
- Use `/skill text-textkit-diag` when undo causes stale layout or crashes.
- Use `/skill text-writing-tools` for Writing Tools interaction with undo.
