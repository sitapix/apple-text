...
- Need wrapper mechanics around `UITextView` / `NSTextView` -> `/skill txt-representable`
- Need to know what SwiftUI `Text` renders or ignores -> stay here
- Need attributed-text type choice itself -> `/skill txt-attributed-string`
...
- Use `/skill txt-representable` for wrapper mechanics and coordinator issues.
- Use `/skill txt-attributed-string` for type-model and conversion strategy.
- Use `/skill txt-markdown` when `PresentationIntent` and Markdown parsing are the real drivers.
