...
- Need type choice or custom attribute scopes -> `/skill text-attributed-string`
- Need exact formatting keys and compatibility -> stay here
- Need semantic text colors rather than general formatting -> `/skill text-colors`
...
- Use `/skill text-attributed-string` for AttributedString vs NSAttributedString decisions.
- Use `/skill text-colors` when the formatting question is mostly about semantic color behavior.
- Use `/skill text-attachments-ref` for inline non-text content instead of pure formatting attributes.
