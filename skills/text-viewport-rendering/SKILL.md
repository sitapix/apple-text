---
name: text-viewport-rendering
description: Use when working with viewport-based text layout, understanding how scroll position affects layout, line fragment geometry, font substitution and fixAttributes, rendering attribute mechanics, Core Text rendering pipeline, emoji rendering, or the differences in how TextKit 1 and TextKit 2 draw text on screen
license: MIT
---
...
- Need full TextKit 2 object reference -> `/skill text-textkit2-ref`
- Need rendering and viewport behavior -> stay here
- Need invalidation semantics rather than rendering pipeline details -> `/skill text-layout-invalidation`
...
- Use `/skill text-textkit2-ref` for the broader TextKit 2 API surface.
- Use `/skill text-layout-invalidation` when the question is about what recomputes, not how it renders.
- Use `/skill text-attachments-ref` when inline views or glyph-like content affect fragment behavior.
