...
- Need attachment rendering -> `/skill txt-attachments`
- Need attributed string conversion -> `/skill txt-attributed-string`
- Need UIViewRepresentable bridging -> `/skill txt-representable`
...
5. **Undo after paste.** Custom paste implementations must go through the normal editing lifecycle (`beginEditing`/`endEditing`) to get proper undo registration. See `/skill txt-undo`.

## Related Skills

- Use `/skill txt-attachments` for attachment sizing, baseline, and view providers.
- Use `/skill txt-attributed-string` for attribute conversion between paste formats.
- Use `/skill txt-undo` for undo registration around paste operations.
