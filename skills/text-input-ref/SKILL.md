---
name: text-input-ref
description: Use when implementing custom text input views, working with UITextInput protocol, UIKeyInput, NSTextInputClient, marked text for CJK input, selection UI, or text interaction APIs — covers the complete text input protocol hierarchy
license: MIT
---
...
- Use `/skill text-representable` when custom input lives inside a SwiftUI wrapper.
- Use `/skill text-writing-tools` when text input requirements intersect with Apple Intelligence editing.
- Use `/skill text-textkit-diag` when the problem is symptom-first rather than protocol-first.
