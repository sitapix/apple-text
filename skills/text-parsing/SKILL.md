---
name: text-parsing
description: Use when choosing between Swift Regex and NSRegularExpression, parsing text for attributed string manipulation, bridging regex results to NSRange for TextKit APIs, or implementing text search/replace patterns — covers performance comparison, type safety, TextKit integration, and when to use which
license: MIT
---
...
- Use `/skill text-foundation-ref` for the wider Foundation text utility catalog.
- Use `/skill text-markdown` when the parsing question is really Markdown rendering or intent handling.
- Use `/skill text-attributed-string` when parsing output feeds attributed-text pipelines.
