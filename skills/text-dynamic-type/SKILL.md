---
name: text-dynamic-type
description: Use when implementing Dynamic Type, scaling custom fonts, supporting accessibility font sizes, using UIFontMetrics, making text views respond to content size category changes, testing Dynamic Type, or understanding what breaks at large accessibility sizes — covers UIKit, AppKit, and SwiftUI text scaling
license: MIT
---
...
- Use `/skill text-views` when sizing depends on which control you chose.
- Use `/skill text-colors` for contrast and semantic color pairing.
- Use `/skill text-writing-tools` when accessibility sizing affects editor integrations.
