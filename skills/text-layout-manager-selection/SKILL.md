---
name: text-layout-manager-selection
description: Use when choosing between NSLayoutManager and NSTextLayoutManager, evaluating TextKit 1 vs 2 performance for a specific use case, deciding whether to migrate to TextKit 2, or needing evidence-based guidance on which layout system is faster — covers real-world benchmarks, Apple's claims vs developer experience, and decision framework
license: MIT
---
...
- Use `/skill text-textkit1-ref` or `/skill text-textkit2-ref` after you know which stack you need.
- Use `/skill text-fallback-triggers` when compatibility mode is driving the decision.
- Use `/skill text-views` when the real question is control choice, not layout-manager internals.
