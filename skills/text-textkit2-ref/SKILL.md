...
- Need to choose between TextKit 1 and 2 -> `/skill text-layout-manager-selection`
- Already committed to TextKit 2 and need exact APIs -> stay here
- Need fragment/rendering behavior specifically -> `/skill text-viewport-rendering`
...
- Use `/skill text-layout-manager-selection` for migration and stack choice.
- Use `/skill text-viewport-rendering` for fragment and rendering-pipeline behavior.
- Use `/skill text-storage` for backing-store and editing-transaction background.
