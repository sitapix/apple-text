...
- Exact formatting attribute catalog needed -> `/skill txt-formatting`

## Core Guidance

Keep this file for the model choice, mutation rules, and conversion boundaries. For custom attribute scopes, paragraph-style recipes, and the full attribute quick reference, use [advanced-patterns.md](references/advanced-patterns.md). For Apple-authored Xcode-backed guidance on the newest Foundation changes, use `/skill txt-apple-docs`.
...
- For custom scopes, paragraph-style recipes, and the attribute-key table, see [advanced-patterns.md](references/advanced-patterns.md).
- Use `/skill txt-formatting` for the full formatting-key catalog.
- Use `/skill txt-swiftui-bridging` when the real issue is what SwiftUI renders or drops.
- Use `/skill txt-markdown` when Markdown parsing is driving the attributed-text shape.
