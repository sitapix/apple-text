---
name: text-spell-autocorrect
description: Use when implementing spell checking, autocorrect, or text completion in text editors — covers UITextChecker, NSSpellChecker, UITextInputTraits configuration, the UITextInteraction private-API trap for custom views, and building custom correction UI
license: MIT
---
...
- Use `/skill text-input-ref` for the full UITextInput protocol and inputDelegate requirements.
- Use `/skill text-interaction` for UITextInteraction setup and gesture handling.
- Use `/skill text-appkit-vs-uikit` for broader platform capability comparison.
- Use `/skill text-views` when the real question is whether to use UITextView vs a custom view.
