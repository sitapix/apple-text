...
- Use `/skill txt-representable` for UIViewRepresentable wrapping when TextEditor's limitations apply.
- Use `/skill txt-apple-docs` for Apple-authored docs access on styled text editing and nearby SwiftUI APIs.
- Use `/skill txt-views` for the full view selection decision tree (which this skill updates for iOS 26).
- Use `/skill txt-attributed-string` for AttributedString model and conversion patterns.
- Use `/skill txt-swiftui-bridging` for what SwiftUI Text renders vs ignores.
