---
name: text-texteditor-26
description: Use when building rich text editing in SwiftUI with TextEditor and AttributedString (iOS 26+) — covers AttributedTextSelection, transformAttributes, formatting constraints, FontResolutionContext, and when UIViewRepresentable is still the right choice
license: MIT
---
...
- Use `/skill text-representable` for UIViewRepresentable wrapping when TextEditor's limitations apply.
- Use `/skill text-views` for the full view selection decision tree (which this skill updates for iOS 26).
- Use `/skill text-attributed-string` for AttributedString model and conversion patterns.
- Use `/skill text-swiftui-bridging` for what SwiftUI Text renders vs ignores.
