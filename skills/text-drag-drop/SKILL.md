---
name: text-drag-drop
description: Use when customizing text drag and drop behavior — UITextDraggable, UITextDroppable, UITextDragDelegate, UITextDropDelegate, UITextDropProposal, drag previews for multi-line text, enabling drag on iPhone, or handling drops in custom UITextInput views
license: MIT
---
...
- Use `/skill text-pasteboard` for copy/paste (synchronous clipboard operations).
- Use `/skill text-interaction` for other gesture and interaction customization.
- Use `/skill text-input-ref` for the UITextInput protocol that custom drag/drop builds on.
- Use `/skill text-attachments-ref` when dropped content becomes inline attachments.
