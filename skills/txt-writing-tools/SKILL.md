...
- Use `/skill txt-textkit2` when Writing Tools behavior depends on TextKit 2 capabilities.
- Use `/skill txt-fallback-triggers` when inline Writing Tools drops into limited mode.
- Use `/skill txt-input` for lower-level custom text input requirements.
