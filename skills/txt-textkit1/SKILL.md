...
- Debugging symptoms before you know the root cause -> `/skill txt-textkit-debug`
...
For the full subclass template, editing lifecycle diagram, and advanced patterns (piece table, CRDT), use `/skill txt-storage`.
...
- Use `/skill txt-textkit-choice` for migration or stack choice.
- Use `/skill txt-fallback-triggers` when TextKit 1 appears unexpectedly.
- Use `/skill txt-storage` for deeper storage-layer behavior underneath the glyph APIs.
