...
Keep this file for the storage architecture, editing lifecycle, and common pitfalls. For custom backing stores (piece table, rope, CRDT), thread-safety patterns, and performance profiling, use [advanced-patterns.md](references/advanced-patterns.md).
...
- Need invalidation behavior after edits -> `/skill txt-layout-invalidation`
...
- Use `/skill txt-layout-invalidation` for what re-renders or recomputes after storage edits.
- Use `/skill txt-textkit1` and `/skill txt-textkit2` for stack-specific APIs.
- Use `/skill txt-textkit-debug` when the symptom matters more than the storage model.
