---
name: text-attributed-string
description: Use when choosing between AttributedString and NSAttributedString, creating custom attributes, converting between types, working with AttributeScopes, Markdown-attributed strings, or NSParagraphStyle — decision guide with patterns for Foundation and UIKit/AppKit attributed text
license: MIT
---
...
- Exact formatting attribute catalog needed -> `/skill text-formatting-ref`

## Core Guidance

Keep this file for the model choice, mutation rules, and conversion boundaries. For custom attribute scopes, paragraph-style recipes, and the full attribute quick reference, use [advanced-patterns.md](advanced-patterns.md).
...
- Use `/skill text-formatting-ref` for the full formatting-key catalog.
- Use `/skill text-swiftui-bridging` when the real issue is what SwiftUI renders or drops.
- Use `/skill text-markdown` when Markdown parsing is driving the attributed-text shape.
