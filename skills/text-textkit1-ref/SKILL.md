...
- Debugging symptoms before you know the root cause -> `/skill text-textkit-diag`
...
- Use `/skill text-layout-manager-selection` for migration or stack choice.
- Use `/skill text-fallback-triggers` when TextKit 1 appears unexpectedly.
- Use `/skill text-storage` for deeper storage-layer behavior underneath the glyph APIs.
