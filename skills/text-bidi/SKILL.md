---
name: text-bidi
description: Use when handling bidirectional text, RTL languages (Arabic, Hebrew), mixed LTR/RTL content, writing direction APIs, embedding vs override controls, Natural Selection (iOS 26), or cursor movement in mixed-direction text
license: MIT
---
...
- Use `/skill text-formatting-ref` for the `.writingDirection` attribute and `baseWritingDirection` on NSParagraphStyle.
- Use `/skill text-input-ref` for `setBaseWritingDirection(_:for:)` in custom UITextInput views.
- Use `/skill text-interaction` for cursor and selection behavior.
- Use `/skill text-appkit-vs-uikit` for platform differences in RTL support.
