---
name: text-markdown
description: Use when working with Markdown in SwiftUI Text, AttributedString Markdown parsing, PresentationIntent for block-level Markdown, custom Markdown attributes, or deciding between native Markdown support and third-party libraries — covers what SwiftUI renders vs ignores, interpretedSyntax options, and MarkdownDecodableAttributedStringKey
license: MIT
---
...
- Regex/parsing mechanics are the main question -> `/skill text-parsing`
...
- Use `/skill text-attributed-string` for attribute-model and conversion choices.
- Use `/skill text-swiftui-bridging` when the real gap is SwiftUI rendering limits.
- Use `/skill text-parsing` when the problem is parser choice more than Markdown rendering semantics.
