---
name: text-core-text
description: Use when dropping from TextKit to Core Text for glyph-level access, custom typesetting, hit testing, font table access, or performance-critical rendering — covers CTLine, CTRun, CTFont, CTFramesetter, coordinate system handling, and bridging patterns between TextKit and Core Text
license: MIT
---
...
- Use `/skill text-textkit2-ref` for the TextKit 2 APIs that sit above Core Text.
- Use `/skill text-textkit1-ref` when NSLayoutManager's glyph APIs are sufficient (no need to drop lower).
- Use `/skill text-viewport-rendering` for how Core Text fits into the rendering pipeline.
- Use `/skill text-attachments-ref` when CTRunDelegate is used for inline non-text content.
