...
- Need storage editing lifecycle -> `/skill txt-storage`
- Need layout invalidation after undo -> `/skill txt-layout-invalidation`
- Need general debugging -> `/skill txt-textkit-debug`
...
- Use `/skill txt-storage` for the editing lifecycle behind undo recording.
- Use `/skill txt-textkit-debug` when undo causes stale layout or crashes.
- Use `/skill txt-writing-tools` for Writing Tools interaction with undo.
