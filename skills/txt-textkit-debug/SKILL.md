...
- Need a code review with severity-ranked findings -> `/skill txt-audit`
- Need direct API reference, not debugging -> use the relevant specialist skill (e.g. `/skill txt-textkit2`, `/skill txt-storage`, `/skill txt-input`, `/skill txt-swiftui-bridging`)
...
| Text resets or flickers in SwiftUI | `updateUIView` called too often or re-creating the text view | `/skill txt-representable` — check for update loops |
...
| Problem only in SwiftUI, works fine in pure UIKit | UIViewRepresentable lifecycle issue | `/skill txt-representable` |
| Custom view (not UITextView/NSTextView) has input issues | UITextInput protocol implementation | `/skill txt-input` |
...
For the complete fallback trigger catalog, detection patterns, and macOS field editor warnings, use `/skill txt-fallback-triggers`.
...
**Note:** This section applies to **custom views that don't inherit from UITextView/NSTextView** — views where you implement UITextInput yourself. If you're using UITextView or NSTextView directly and have input problems, the issue is usually in your delegate or configuration, not in the input protocol. For full UITextInput implementation guidance, use `/skill txt-input`.
...
| Text view resets content on every SwiftUI update | `updateUIView` re-setting text unconditionally | `/skill txt-representable` — guard against redundant updates |
...
1. **"It works in the playground"** — Playgrounds wrap text in unusual containers. Reproduce in a real app target before debugging.

2. **Comparing `String.count` to `NSString.length`** — they diverge on emoji, ZWJ sequences, and combining marks. A "wrong range" bug is often this in disguise. Always normalize via `(text as NSString).length` or `NSRange(swiftRange, in: text)`.

3. **Calling `ensureLayout(for: textContainer)` to "fix" stale layout** — forces full-document layout in TextKit 1 and defeats viewport optimization in TextKit 2. Use range-scoped `ensureLayout(forCharacterRange:)` or trigger via the natural draw path.

4. **Editing `textStorage` from a background queue** — NSTextStorage is main-thread-confined. Background mutations cause sporadic crashes with no obvious stack frame in the offending code.
...
6. **Assuming layout invalidation propagates immediately** — invalidation marks regions; layout is recomputed lazily on next display. If a measurement queries layout *before* the next pass, it sees stale data. Force the boundary with `ensureLayout(forCharacterRange:)` or wait for the next runloop.

7. **Setting a TextKit 2 attribute and expecting it to render** — TK2 attributes attach to layout fragments, not character ranges. Use `setRenderingAttributes(_:for:)` on the layout manager, not `addAttribute` on the storage.

## Related Skills

- Use `/skill txt-fallback-triggers` for the compatibility-mode catalog.
- Use `/skill txt-layout-invalidation` for invalidation details.
- Use `/skill txt-audit` when you want repository findings ordered by severity.
