...
- Need text view selection or wrapping -> `/skill text-views` or `/skill text-representable`
- Need rendering overlays for highlighting -> `/skill text-viewport-rendering`
- Need attributed string patterns -> `/skill text-attributed-string`
...
- Use `/skill text-viewport-rendering` for custom rendering overlay patterns.
- Use `/skill text-attributed-string` for attribute-based highlighting choices.
- Use `/skill text-undo` when find-replace undo grouping is wrong.
