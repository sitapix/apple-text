...
- Need parser choice guidance -> `/skill text-parsing`
- Need the exact utility API or compatibility details -> stay here
- Need attributed-text model guidance instead of utilities -> `/skill text-attributed-string`
...
- Use `/skill text-parsing` for Swift Regex vs `NSRegularExpression` choice.
- Use `/skill text-markdown` when parsing feeds Markdown-rendering workflows.
- Use `/skill text-attributed-string` when utility output becomes attributed content.
