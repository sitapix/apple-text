...
- Use `/skill txt-pasteboard` for copy/paste (synchronous clipboard operations).
- Use `/skill txt-interaction` for other gesture and interaction customization.
- Use `/skill txt-input` for the UITextInput protocol that custom drag/drop builds on.
- Use `/skill txt-attachments` when dropped content becomes inline attachments.
