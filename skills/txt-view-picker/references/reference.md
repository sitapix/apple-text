...
Every text input and display view available on Apple platforms: SwiftUI, UIKit, and AppKit. Use the matrix below to narrow down, then jump to the per-view section for detail.

## Quick Comparison Matrix

| View | Editable | Multi-line | Rich text | Find/Replace | Attachments | Min OS |
|------|----------|-----------|-----------|--------------|-------------|--------|
| `Text` | ❌ | ✅ | Display only (`AttributedString`, inline Markdown subset) | ❌ | ❌ | iOS 13 |
| `TextField` | ✅ | iOS 16+ via `axis: .vertical` | ❌ (plain `String` only) | ❌ | ❌ | iOS 13 |
| `SecureField` | ✅ | ❌ | ❌ | ❌ | ❌ | iOS 13 |
| `TextEditor` (plain) | ✅ | ✅ | ❌ until iOS 26 | ✅ via `.findNavigator` (iOS 16+) | ❌ | iOS 14 |
| `TextEditor` (rich) | ✅ | ✅ | ✅ `AttributedString` binding | ✅ | ✅ (in scope) | iOS 26 |
| `Label` | ❌ | ✅ | Same as `Text` + leading icon | ❌ | ❌ | iOS 14 |
| `UILabel` | ❌ | ✅ | `NSAttributedString` | ❌ | Display-only (image attachments) | iOS 2 |
| `UITextField` | ✅ | ❌ | `NSAttributedString` placeholder | ❌ | ❌ | iOS 2 |
| `UITextView` | ✅ | ✅ | Full `NSAttributedString` + TextKit 1/2 | ✅ via `UIFindInteraction` (iOS 16+) | ✅ | iOS 2 |
| `NSTextField` | ✅ (configurable) | ✅ when wrapped | `NSAttributedString` | ❌ (use field editor) | ❌ | macOS 10.0 |
| `NSSecureTextField` | ✅ | ❌ | ❌ | ❌ | ❌ | macOS 10.0 |
| `NSTextView` | ✅ | ✅ | Full `NSAttributedString` + TextKit 1/2 | ✅ via `NSTextFinder` | ✅ | macOS 10.0 |

**Decision shortcuts:**
- Plain `String` input, single line → `TextField` (SwiftUI) or `UITextField` / `NSTextField`
- Plain `String` input, growing → `TextField(axis: .vertical)`
- Long-form plain editing → `TextEditor`
- Rich-text editing → `TextEditor` on iOS 26+; otherwise wrap `UITextView`/`NSTextView` (`/skill txt-wrap-textview`)
- Read-only display, simple styling → `Text` / `UILabel` / `NSTextField` (non-editable)
- Read-only display, attachments or TextKit features → `UITextView` (read-only) / `NSTextView` (read-only)
