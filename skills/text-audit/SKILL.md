---
name: text-audit
description: Use when auditing Apple text system code for anti-patterns, fallback triggers, editing lifecycle bugs, deprecated APIs, or Writing Tools compatibility problems. Runs a focused TextKit review via the textkit-auditor agent. Also useful as a standalone inline checklist when no agent is available.
license: MIT
context: fork
...
- Need symptom-first debugging without a code scan -> `/skill text-textkit-diag`
...
- Use `/skill text-textkit-diag` for symptom-first troubleshooting.
- Use `/skill text-fallback-triggers` for the full fallback trigger catalog.
- Use `/skill apple-text` when the prompt is too broad to treat as a code review yet.
- Use `/skill text-storage` for deeper editing lifecycle context behind P0 items 2-5.
