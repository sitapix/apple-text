...
- Need text view selection or wrapping -> `/skill txt-views` or `/skill txt-representable`
- Need rendering overlays for highlighting -> `/skill txt-viewport-rendering`
- Need attributed string patterns -> `/skill txt-attributed-string`
...
- Use `/skill txt-viewport-rendering` for custom rendering overlay patterns.
- Use `/skill txt-attributed-string` for attribute-based highlighting choices.
- Use `/skill txt-undo` when find-replace undo grouping is wrong.
