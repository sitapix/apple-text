...
- Use `/skill txt-formatting` for the `.writingDirection` attribute and `baseWritingDirection` on NSParagraphStyle.
- Use `/skill txt-input` for `setBaseWritingDirection(_:for:)` in custom UITextInput views.
- Use `/skill txt-interaction` for cursor and selection behavior.
- Use `/skill txt-appkit-vs-uikit` for platform differences in RTL support.
