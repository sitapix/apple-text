---
name: text-fallback-triggers
description: Use when TextKit 2 falls back to TextKit 1 unexpectedly, when auditing code for fallback risk, when needing to understand what triggers compatibility mode, or when using NSTextStorage with TextKit 2 without causing fallback — complete catalog of every known fallback trigger with safe alternatives
license: MIT
---
...
- Need a symptom-first debugger -> `/skill text-textkit-diag`
- Need the exact fallback trigger catalog -> stay here
- Need to choose TextKit 1 on purpose -> `/skill text-layout-manager-selection`
...
- Use `/skill text-textkit-diag` for broader debugging around fallback symptoms.
- Use `/skill text-layout-manager-selection` when compatibility mode pressure means TextKit 1 may be the right explicit choice.
- Use `/skill text-audit` when you want repository findings ranked by severity.
