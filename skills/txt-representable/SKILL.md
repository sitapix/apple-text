...
- Need cross-framework type/rendering limits instead of wrapper mechanics -> `/skill txt-swiftui-bridging`
...
- Use `/skill txt-views` when you still need to choose the view class.
- Use `/skill txt-swiftui-bridging` for type-scope and rendering-boundary questions.
- Use `/skill txt-textkit-choice` when wrapper behavior depends on TextKit 1 vs 2.
