...
- Use `/skill txt-input` for the full UITextInput protocol and inputDelegate requirements.
- Use `/skill txt-interaction` for UITextInteraction setup and gesture handling.
- Use `/skill txt-appkit-vs-uikit` for broader platform capability comparison.
- Use `/skill txt-views` when the real question is whether to use UITextView vs a custom view.
