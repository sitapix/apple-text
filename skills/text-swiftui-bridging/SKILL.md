...
- Need wrapper mechanics around `UITextView` / `NSTextView` -> `/skill text-representable`
- Need to know what SwiftUI `Text` renders or ignores -> stay here
- Need attributed-text type choice itself -> `/skill text-attributed-string`
...
- Use `/skill text-representable` for wrapper mechanics and coordinator issues.
- Use `/skill text-attributed-string` for type-model and conversion strategy.
- Use `/skill text-markdown` when `PresentationIntent` and Markdown parsing are the real drivers.
