...
description: Use when integrating Writing Tools (iOS 18+ / macOS 15+), configuring writingToolsBehavior, implementing UIWritingToolsCoordinator for custom text engines, or debugging Writing Tools not appearing — covers native text view integration, custom view adoption, protected ranges, and iOS 26 coordinator API
license: MIT
---
...
- Use `/skill text-textkit2-ref` when Writing Tools behavior depends on TextKit 2 capabilities.
- Use `/skill text-fallback-triggers` when inline Writing Tools drops into limited mode.
- Use `/skill text-input-ref` for lower-level custom text input requirements.
