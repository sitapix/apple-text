---
name: text-pasteboard
description: Use when handling copy, cut, and paste in text editors, sanitizing pasted rich text, implementing custom pasteboard types, stripping formatting on paste, handling pasted images as text attachments, or bridging NSItemProvider with NSAttributedString — covers UIPasteboard and NSPasteboard patterns
license: MIT
---
...
- Need attachment rendering -> `/skill text-attachments-ref`
- Need attributed string conversion -> `/skill text-attributed-string`
- Need UIViewRepresentable bridging -> `/skill text-representable`
...
5. **Undo after paste.** Custom paste implementations must go through the normal editing lifecycle (`beginEditing`/`endEditing`) to get proper undo registration. See `/skill text-undo`.

## Related Skills

- Use `/skill text-attachments-ref` for attachment sizing, baseline, and view providers.
- Use `/skill text-attributed-string` for attribute conversion between paste formats.
- Use `/skill text-undo` for undo registration around paste operations.
