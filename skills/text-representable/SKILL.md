...
description: Use when wrapping UITextView or NSTextView in SwiftUI via UIViewRepresentable or NSViewRepresentable — covers the Coordinator pattern, two-way binding, auto-sizing, focus bridging, cursor jump bug, infinite update loops, rendering layer, toolbar integration, environment bridging, limitations, and third-party alternatives (STTextView, RichTextKit, Textual)
license: MIT
---
...
- Need cross-framework type/rendering limits instead of wrapper mechanics -> `/skill text-swiftui-bridging`
...
- Use `/skill text-views` when you still need to choose the view class.
- Use `/skill text-swiftui-bridging` for type-scope and rendering-boundary questions.
- Use `/skill text-layout-manager-selection` when wrapper behavior depends on TextKit 1 vs 2.
