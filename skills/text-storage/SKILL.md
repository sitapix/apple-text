---
name: text-storage
description: Use when working with NSTextStorage subclassing, editing lifecycle, NSTextContentStorage, NSTextContentManager, or understanding how text model layers relate — covers processEditing flow, delegate hooks, custom backing stores, and TextKit 1/2 storage architecture
license: MIT
---
...
- Need invalidation behavior after edits -> `/skill text-layout-invalidation`
...
- Use `/skill text-layout-invalidation` for what re-renders or recomputes after storage edits.
- Use `/skill text-textkit1-ref` and `/skill text-textkit2-ref` for stack-specific APIs.
- Use `/skill text-textkit-diag` when the symptom matters more than the storage model.
