...
- Need text view selection and wrapping -> `/skill txt-views`
- Need text input protocol details -> `/skill txt-input`
- Need Writing Tools coordinator -> `/skill txt-writing-tools`
- Need copy/paste behavior -> `/skill txt-pasteboard`
...
class CustomEditorView: UIView, UITextInput {
    ...
    // (see txt-input for the full protocol)
}
```
...
- Use `/skill txt-input` for the full `UITextInput` protocol.
- Use `/skill txt-writing-tools` for Writing Tools interaction with menus.
- Use `/skill txt-pasteboard` for copy/paste customization.
- Use `/skill txt-views` for view selection decisions.
