---
name: text-layout-invalidation
description: Use when text layout is stale, not updating, or you need to understand what triggers layout recalculation — covers TextKit 1 and TextKit 2 invalidation models, ensureLayout, viewport layout, rendering attribute updates, and forced re-rendering patterns
license: MIT
---
...
- Need storage/editing lifecycle background -> `/skill text-storage`
...
- Use `/skill text-textkit-diag` for broader troubleshooting.
- Use `/skill text-storage` when invalidation questions are really about editing lifecycle.
- Use `/skill text-textkit2-ref` for direct API details around layout fragments and viewport behavior.
