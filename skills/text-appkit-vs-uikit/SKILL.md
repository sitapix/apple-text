---
name: text-appkit-vs-uikit
description: Use when comparing NSTextView and UITextView capabilities, porting text code between macOS and iOS, or wondering what niche features AppKit text has that UIKit lacks — covers field editor, text tables, font panel, ruler, spell/grammar checking, text completion, services, printing, and architecture differences
license: MIT
---
...
- Use `/skill text-views` when the real question is which view class to adopt.
- Use `/skill text-representable` when SwiftUI wrapping is part of the platform decision.
- Use `/skill text-writing-tools` for Apple Intelligence editor differences.
