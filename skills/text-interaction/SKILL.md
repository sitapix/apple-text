...
description: Use when customizing text editor interactions — UITextInteraction, UIEditMenuInteraction context menus, link tap handling, custom selection behavior, text cursor appearance, long press actions, or overriding default text view gestures — covers UIKit text interaction patterns
license: MIT
---
...
- Need text view selection and wrapping -> `/skill text-views`
- Need text input protocol details -> `/skill text-input-ref`
- Need Writing Tools coordinator -> `/skill text-writing-tools`
- Need copy/paste behavior -> `/skill text-pasteboard`
...
class CustomEditorView: UIView, UITextInput {
    ...
    // (see text-input-ref for the full protocol)
}
```
...
- Use `/skill text-input-ref` for the full `UITextInput` protocol.
- Use `/skill text-writing-tools` for Writing Tools interaction with menus.
- Use `/skill text-pasteboard` for copy/paste customization.
- Use `/skill text-views` for view selection decisions.
