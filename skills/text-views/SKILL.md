...
For the actual TextKit 1 vs 2 choice, jump to `/skill text-layout-manager-selection`.
...
- For wrapping `UITextView` or `NSTextView` in SwiftUI, use `/skill text-representable`.
- For TextKit 1 vs 2 architecture, use `/skill text-textkit1-ref` or `/skill text-textkit2-ref`.
- For a migration or performance decision, use `/skill text-layout-manager-selection`.
- For debugging weird editor behavior, use `/skill text-textkit-diag`.
